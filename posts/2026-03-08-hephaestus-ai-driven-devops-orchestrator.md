...
1. **Core (Konstytucja)**: AI dostaje nienaruszalne reguły projektu, np. jak nazywać pliki, jakie technologie stosować. Agent "Dev" nie wymyśla architektury od nowa - trzyma się mojej (zapisanego np. w `GEMINI.md`).
...
- **Backend**: Cały silnik to **FastAPI** (Python 3.12). Szybki, asynchroniczny i potężny. Dba o uwierzytelnianie, zarządza projektami w dedykowanym widoku (dashboardzie z szablonami Jinja2) i wystawia asynchroniczne endpointy chwytające Webhooki.
